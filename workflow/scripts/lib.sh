...
# Set up the environment for disciplined logging, caching, and for a conda environment to use for the data preparation.
# Before loading the library you should set
#
#  * LOG_FILE_PREFIX to a prefix that will be extended by a date tag for a logfile. Usually you can use
#       LOG_FILE_PREFIX="$BASH_SOURCE."
#    for this. All logging will then be done to standard input and error but copied into the logfile.
#
initialize() {
    LIB_DIR=$(dirname "$BASH_SOURCE")
    REPO_BASE_DIR=$(readlink -f "$LIB_DIR/../")
    REFERENCE_DATA_DIR=$(readlink -f "$REPO_BASE_DIR/data")
    CACHE_DIR="$REPO_BASE_DIR/cache"

    # From https://stackoverflow.com/questions/18460186/writing-outputs-to-log-file-and-console
    if [[ ! -v LOG_FILE_PREFIX || -w "$LOG_FILE_PREFIX" ]]; then
        echo "No LOG_FILE_PREFIX variable set! Please use the scripts execution directory" >> /dev/stderr
        exit 1
    fi
    LOG_FILE="${LOG_FILE_PREFIX}"$(date -Iminutes)
    exec >  >(set +vx; while read -r line; do printf '%s %s\n' "$(date --rfc-3339=seconds)" "$line" | tee -a "$LOG_FILE"; done)
    exec 2> >(set +vx; while read -r line; do printf '%s %s\n' "$(date --rfc-3339=seconds)" "$line" | tee -a "$LOG_FILE"; done >&2)

    mkdir -p "$CACHE_DIR"
    $(dirname "$CONDA_EXE")/activate "${CONDA_ENVIRONMENT_NAME:-setup-reference-data}"
}

# Directly initialize the environment
initialize
...
# Retrieve a file via wget, but only if there is no cached copy.
cachedWget() {
    ...
    if [[ ! -f "$cachedFile" ]]; then
        ...
        date -r "$cachedFile" > "$cachedFile.timestamp"
    else
        echo "Cached file '$cachedFile' exists ("$(cat "$cachedFile.timestamp")". Skipping re-download ..." >> /dev/stderr
    fi
    ln -s "$cachedFile" "$targetFile"
    ln -s "$cachedFile.timestamp" "$targetFile.timestamp"
}

checkMD5() {
    local file="${1:?No file to check given}"
    local md5file="${2:?No MD5 file given}"
    # The application on the standard input requires that the filename is '-'.
    cat "$file" | md5sum --strict -c <(cat "$md5file" | perl -pe 's/^(\S+).*/$1\t-/')
}
...
download_phiX() {
    ...
    # Note that we do *not* use a temporary file for the tar.gz reference here. The cacheWget command symlinks the
    # downloaded file and timestamp-file to this filename (.timestamp) and this will server as a log of what happened.
    cachedWget \
        "ftp://igenome:G3nom3s4u@ussd-ftp.illumina.com/PhiX/Illumina/RTA/PhiX_Illumina_RTA.tar.gz" \
        "$targetDir/PhiX_Illumina_RTA.tar.gz" \
        "$md5fileDir"
...
download_core_ref_GRCh38_hla_decoy_ebv() {
    ...
    cachedWget \
        ...
        "$targetDir/core_ref_GRCh38_hla_decoy_ebv.tar.gz" \
        "$md5fileDir"
...
pathMetadataVariables() {
    local assemblyVariantDir="${1:?No assembly variant directory}"
    local assemblyVariant=$(basename "$assemblyVariantDir")
    local primaryAssembly=$(basename $(basename "$assemblyVariantDir"))
    local species=$(basename $(basename $(basename "$assemblyVariantDir")))
    echo "speciesName='$species'"
    echo "primaryAssemblyName='$primaryAssembly'"
    echo "assemblyVariantName='$assemblyVariant'"
}

getBwaVersion() {
    bwa 2>&1 | grep Version | cut -f 2 -d ' ' | cut -f 1 -d '-'
}
